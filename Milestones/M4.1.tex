...
experiments. The fowolling tables list the simulation codes (backengines) that
...
\begin{enumerate}
  ...
  \item Small-angle x-ray scattering from high power laser excited overdense
    ...
    \begin{tabular}{l|l|l|l}
      ...
      & \cite{Bussmann2013} & \ref{sec:interface_pmi_pic} \\
      %
      Photon scattering &  paraTaxis & \cite{parataxis_github} &
      \ref{sec:interface_diffr_parataxis} \\
      %
      Photon detection &  X-CSIT & \cite{Joy2015} &
      \ref{sec:interface_det_xcsit} \\
      %
  ...
  \item Warm dense matter production through high energy laser shock compression
    ...
    \begin{tabular}{l|l|l|l}
      ...
      %
      Photon source & ?? &
      ?? & \ref{sec:interface_source_esrf} \\
      %
      Propagation &  Shadow/XRT &
      \cite{shadow,xrt} & \ref{sec:interface_prop_shadow},
      \ref{sec:interface_prop_xrt} \\
      %
      Photon-Matter Interaction & Esther, Multi2D
      & \cite{Colombier2005, Ramis2009} & \ref{sec:interface_pmi_radhydro} \\
      %
      Photon absorption &  ?? & \cite{XAFS} &
      \ref{sec:interface_diffr_xafs} \\
      \hline
      \hline
    \end{tabular}
  }
...
\section{\textit{Backengines}, \textit{Calculators}, and interfaces}
...
The EUCALL github repository www.github.com/eucall-software
is the single-entry point to \texttt{simex\_platform} and its integrated
\textit{Calculators} plus additional \textit{Backengines} that have not become a
fully integrated part of \texttt{simex\_platform} yet.
...
\subsubsection{ESRF source code ???}
\begin{description}
  \item[Method:] ??
  \item[Data format (output):]
\end{description}
...
\subsubsection{WPG/SRW\ref{sec:interface_prop_wpg}}
\begin{description}
...
\begin{description}
  \item[Method:] Molecular Dynamics and ab-initio
    electronic structure in intense x-ray fields
  \item[Data format (output):]
\end{description}
{\scriptsize%
\begin{tabular}{l|l}
...
\subsubsection{X-CSIT}
\begin{description}
  \item[Method:] Combined particle, charge, and electronics simulation
  \item[Data format (output):] Detector response to the signal, e.g. photons per pixel in an area detector.
\end{description}
{\scriptsize%
  \begin{tabular}{l|l}
    \hline
