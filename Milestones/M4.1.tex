...
sources. Users can assemble a virtual experiment through combination
...
electron laser), propagation of radiation the source to the
...
interaction, and detection. By default, \texttt{simex\_platform} is equipped
with a number of such codes and user interfaces to ease setup and execution of
simulation runs. A ligthweight abstraction mechanism and interface templates
...
stand-alone fashion with more or less idealized parameters and initial
conditions.
...
\texttt{simex\_platform} is implemented in the python programming language
version 2.7. The software requirements for
\texttt{simex\_platform} as well as for the backengine calculators that ship
with it are given in the documentation and in the appendix.
%
...
detection, and data analysis. In \texttt{simex\_platform}
...
framework \texttt{simS2E} \cite{Yoon2016, simS2E_doc}. Recently, we have started
...
experiments. The fowolling tables list the simulation codes
...
\begin{enumerate}
  ...
  \item Warm dense matter production through high energy laser shock compression
    ...
    \begin{tabular}{l|l|l|l}
      ...
      %
      Propagation &  Shadow &
      \cite{Rio2013} & \ref{sec:interface_prop_shadow}\\
      %
...
%
\subsubsection{Shadow\label{sec:interface_prop_shadow}}
\begin{description}
...
\begin{description}
  ...
  \item[Data format:] openPMD incl. domiain extension for PIC.
\end{description}
%
\subsubsection{Esther\label{sec:interface_pmi_esther}}
\begin{description}
  \item[Data format descriptor]: openPMD
\end{description}
...
\begin{description}
  ...
  \item[Data format:] openPMD particles
\end{description}
